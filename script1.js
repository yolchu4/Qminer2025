...
async function loadPredictionFromJSON() {
  try {
    // In a real scenario, this would be loaded from the server
    // For demo purposes, we'll simulate loading from a file
    ...
    console.log('Prediction results loaded successfully');
    return predictionData;
  } catch (error) {
    console.error('Error loading prediction results:', error);
    // Simulate prediction if file doesn't exist
    const simulatedPrediction = simulatePrediction();
    updateUIWithPrediction(simulatedPrediction);
...
function setupEventListeners() {
  ...
  // Predict values - NEW VERSION WITH JSON
  document.getElementById('predict').addEventListener('click', async () => {
    status.innerText = 'در حال پیش‌بینی...';
    
    try {
      // Step 1: Save current input values to JSON
      const inputData = saveInputToJSON();
      
      // Step 2: Simulate model processing (in real scenario, this would be done by server)
      setTimeout(async () => {
        // Step 3: Load prediction results from JSON
        const predictionData = await loadPredictionFromJSON();
        
        status.innerText = 'پیش‌بینی تکمیل شد ✅';
        
        // Show prediction summary
        const mu = Object.values(predictionData.numeric_predictions);
        const std = Object.values(predictionData.uncertainties);
        drawSummaryChart(mu, std);
        
      }, 2000); // Simulate 2 second delay for model processing
